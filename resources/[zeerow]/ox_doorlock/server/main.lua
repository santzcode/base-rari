...
lib.addCommand('doorlock', {
	...
	if not IsPlayerInGroup({ source = source },'Admin') then
		return lib.notify(source, { type = 'error', icon = 'lock', description = 'Você não tem permissão para usar este comando.' })
	end
