...
export const UserVehiclesModal = () => {
  ...
  return (
    <div className="w-[1428px] h-[707px] rounded-[10px] border border-solid border-[#3A3A3A] bg-neutral-900 absolute z-50 -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2 flex flex-col px-[45px] pb-[28px]">
      ...
      >
        {modal.data.map((item: PersonalVehicle, index: number) => (
          <VehicleCell key={index} data={item} />
        ))}
        <button
          onClick={() => modal.setVisible("adduservehicle", selectedUser.data)}
...
const VehicleCell = ({ data }: { data: PersonalVehicle }) => {
  const selectedUser = TempUserContext();
  const modal = useModal();
  return (
    <li
      onClick={() => modal.setVisible("infopersonalvehicle", data)}
      className="w-[209px] h-[270px] border border-solid border-white/5 rounded-[5px] bg-white/1 p-[15px] flex flex-col cursor-pointer hover:bg-white/3"
    >
