...
export const PlayerList = () => {
  ...
  return (
    <div className="w-[804px] flex-none h-full bg-white/2 rounded-[5px] px-[15px]">
...
const List = () => {
  ...
  const renderList = (): Player[] => {
    return sortedList().filter(
      (e) =>
        e.name.toLowerCase().startsWith(search.current.toLowerCase()) ||
        String(e.id).includes(search.current.toLowerCase())
    );
  };
