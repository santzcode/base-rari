...
RegisterNetEvent('bs_spray:setSprays')
AddEventHandler('bs_spray:setSprays', function(s)
    SPRAYS = s
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if wantedSprayLocation and wantedSprayRotation and IsSpraying then
            ...
            if currentSprayRotation ~= wantedSprayRotation or not currentSprayRotation or ttl < 0 or currentComputedRotation == vector3(0,0,0) then
                ttl = 30
            ...
            -- Reset rotation computation when not spraying
...
function GetRotationThruCamera(location, normal)
    ...
    -- Correção para alinhamento correto na parede
...
function DrawSpray(scaleformHandle, sprayData, meth)
    if scaleformHandle and HasScaleformMovieLoaded(scaleformHandle) then
        if not IsPauseMenuActive() then
            ...
            local text = sprayData.text or "GRAFITE"
            local color = sprayData.color or "ffffff"
            local font = sprayData.font or "graffiti1"
            local scale = sprayData.scale or 1.0
            
            -- Clean and format the text
            text = string.upper(text)
            
            -- Apply font formatting with proper color
            local formattedText = "<FONT color='#" .. color .. "' FACE='" .. font .. "' SIZE='1.0'>" .. text .. "</FONT>"
            
            PushScaleformMovieFunction(scaleformHandle, "SET_PLAYER_NAME")
            PushScaleformMovieFunctionParameterString(formattedText)
            PopScaleformMovieFunctionVoid()

            if not sprayData.realRotation then
                wantedSprayLocation = sprayData.location
                wantedSprayRotation = sprayData.rotation
                -- Force rotation recalculation when starting a new spray
                currentSprayRotation = nil
            end
