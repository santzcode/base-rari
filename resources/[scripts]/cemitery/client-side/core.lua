...
AddEventHandler("cemitery:initBody",function()
	...
	if not Peds then
		...
		if LoadModel(List[Hash]) then
			...
			TriggerEvent("Notify","amarelo","Parece que estão efetuando uma limpeza em um dos túmulos, guarde essa informação e veja se você encontra alguns objetos de valor.",10000,"Observação")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
