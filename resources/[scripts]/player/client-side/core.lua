...
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYER:CARREGAR NO H
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("player:Commands")
local carryCooldown = 0
RegisterCommand("+Carry",function()
	local Ped = PlayerPedId()
	if GetGameTimer() < carryCooldown then return end
	carryCooldown = GetGameTimer() + 500
	if not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and LocalPlayer["state"]["Route"] < 900000 then
		TriggerServerEvent("player:carryHotkey")
	end
end)
RegisterCommand("-Carry",function() end)
RegisterKeyMapping("+Carry","Carregar pessoa próxima.","keyboard","H")
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYER:COMMANDS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("player:Commands")
...
-- SYNCDOORSOPTIONS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("player:syncTrunkDoor")
AddEventHandler("player:syncTrunkDoor",function(Network,Active)
	if NetworkDoesNetworkIdExist(Network) then
		local Vehicle = NetToEnt(Network)
		if DoesEntityExist(Vehicle) then
			if Active == "open" then
			...
			if IsPedArmed(Ped,6) and GetGameTimer() >= ShotDelay then
				...
				if IsPedShooting(Ped) then
					...
					if not IsPedCurrentWeaponSilenced(Ped) then
						if (losSantos:isPointInside(Coords) or sandyShores:isPointInside(Coords) or paletoBay:isPointInside(Coords)) and not vRP.IsPlayerACop() then
