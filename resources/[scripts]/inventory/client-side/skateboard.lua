...
RCCar.HandleKeys = function(distanceCheck)
	...
	if distanceCheck < 100.0 then
		local overSpeed = (GetEntitySpeed(RCCar.Entity)*3.6) > 35
		...
		if Attached and (IsControlPressed(0,32) and not IsControlPressed(0, 33) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 9, 1)
		end

		if Attached and IsControlPressed(0, 22) then
			-- Jump system
		...
		if Attached and (IsControlJustReleased(0, 32) or IsControlJustReleased(0, 33) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 6, 2500)
		end

		if Attached and (IsControlPressed(0, 33) and not IsControlPressed(0, 32) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 22, 1)
		end

		if Attached and (IsControlPressed(0, 34) and IsControlPressed(0, 33) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 13, 1)
		end

		if Attached and (IsControlPressed(0, 35) and IsControlPressed(0, 33) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 14, 1)
		end

		if Attached and (IsControlPressed(0, 32) and IsControlPressed(0, 33) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 30, 100)
		end

		if Attached and (IsControlPressed(0, 34) and IsControlPressed(0, 32) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 7, 1)
		end

		if Attached and (IsControlPressed(0, 35) and IsControlPressed(0, 32) and not overSpeed) then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 8, 1)
		end

		if Attached and (IsControlPressed(0, 34) and not IsControlPressed(0, 32) and not IsControlPressed(0, 33) and not overSpeed )then
			TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 4, 1)
		end

		if Attached and (IsControlPressed(0, 35) and not IsControlPressed(0, 32) and not IsControlPressed(0, 33) and not overSpeed) then
...
RCCar.Attach = function(param)
	...
	elseif param == 'pick' then
		...
		TriggerServerEvent("TakeItemSkate")
