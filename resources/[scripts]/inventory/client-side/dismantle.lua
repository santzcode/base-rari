	...
	while true do
		if Active and not Vehicle then
			...
			if #(GetEntityCoords(Ped) - Dismantle[Selected]["xyz"]) <= 75 then
				...
				SetVehicleExtraColours(Network,147,147)
				...
				SetVehicleCustomPrimaryColour(Network,RandColor(),RandColor(),RandColor())
				SetVehicleCustomSecondaryColour(Network,RandColor(),RandColor(),RandColor())
			end
		end
		Wait(1000)
	end
...
AddEventHandler("dismantle:Dispatch",function()
	...
	for Number = 1,Amounts do
		...
		if HitZ and HitSafe then
			local Application,Network = vRPS.CreatePed(Peds[OtherPeds],SafeCoords["x"],SafeCoords["y"],SafeCoords["z"])
			if Application then
