...
RegisterNUICallback("Send",function(Data,Callback)
	if not IsPedArmed(PlayerPedId(),6) and MumbleIsConnected() then
...
AddEventHandler("inventory:Update",function(Action,invPeso,invMaxpeso,chestPeso,chestMaxpeso)
	SendNUIMessage({ action = Action, inVehicle = GetVehiclePedIsUsing(PlayerPedId()) ~= 0 })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("Inventory",function()
	local Ped = PlayerPedId()
	if not IsPauseMenuActive() and GetEntityHealth(PlayerPedId()) > 100 and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not LocalPlayer["state"]["Carry"] and not IsPlayerFreeAiming(PlayerId()) then
		if IsPedInAnyVehicle(Ped) then
			local Vehicle = GetVehiclePedIsUsing(Ped)
			if GetVehicleClass(Vehicle) ~= 8 then
				if GetVehicleDoorLockStatus(Vehicle) <= 1 then
					TriggerEvent("sounds:Private","houselock",0.6)
					TriggerEvent("chest:Open",GetVehicleNumberPlateText(Vehicle)..":"..GetEntityArchetypeName(Vehicle),"Glove")
				else
					Inventory = true
					SetNuiFocus(true,true)
					SetCursorLocation(0.5,0.5)
					SendNUIMessage({ action = "Open", inVehicle = true })
				end
			else
				Inventory = true
				SetNuiFocus(true,true)
...
function Creative.IsInWater()
	...
	if IsEntityInWater(Ped) then
        ...
        local isWater, waterZ = GetWaterHeightNoWaves(Coords.x, Coords.y, Coords.z - 1.0, 0.0)
		local Distance = math.abs(waterZ - groundZ)
        if isWater and Distance < 20.0 then
			return true
        end
	end
	return false
...
function Creative.InfoWeapon(Type)
	...
	local Ammos = {
		...
		["WEAPON_RAYPISTOL_AMMO"] = {
			"WEAPON_RAYPISTOL"
		},
		["WEAPON_GRENADELAUNCHER_AMMO"] = {
...
RegisterNUICallback("Drops",function(Data,Callback)
	if not TakeWeapon and not StoreWeapon and MumbleIsConnected() then
		PlaySoundFrontend(-1, "CLICK_BACK", "WEB_NAVIGATION_SOUNDS_PHONE", 1)
		vSERVER.Drops(Data["slot"],Data["item"],Data["amount"])
	end
	Callback("Ok")
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback("Pickup",function(Data,Callback)
	if MumbleIsConnected() then
		if vSERVER.Pickup(Data["route"],Data["id"],Data["target"],Data["amount"]) then
	...
	if Inventory then
		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			...
			for Index,v in pairs(Drops[Route]) do
				if #(vec3(Coords["x"],Coords["y"],GroundZ) - v["Coords"]) <= 1.25 then
		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			...
			for _,v in pairs(Drops[Route]) do
				if #(Coords - v["Coords"]) <= 50 then
					TimeDistance = 1
					DrawMarker(22,v["Coords"]["x"],v["Coords"]["y"],v["Coords"]["z"] - 0.75,0.0,0.0,0.0,0.0,180.0,0.0,0.25,0.35,0.25,59, 130, 246,500,1,1,1,1)
				end
			end
		end
		Wait(TimeDistance)
...
AddEventHandler("CEventGunShot",function(_,OtherPeds)
	local Ped = PlayerPedId()
	if Ped == OtherPeds and not LocalPlayer["state"]["Pmesp"] and GetGameTimer() >= ShotDelay and Weapon ~= "WEAPON_MUSKET" and Weapon ~= "WEAPON_SAUER" then
...
-- INVENTORY:KEY
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("inventory:Key",function()
	local Keys = vSERVER.UserKeys()
	if Keys["Vehicles"][1] then
		for _,v in pairs(Keys["Vehicles"]) do
			exports["dynamic"]:AddButton(VehicleName(v["vehicle"]).." - "..v["plate"],"Criar chave cópia por <green>R$"..parseFormat(VehiclePrice(v["vehicle"])*0.02).."</green> Reais.","inventory:Key",(VehiclePrice(v["vehicle"])*0.02).."-vehkey-"..v["plate"],"Vehicles",true)
		end
		exports["dynamic"]:SubMenu("Veiculos","lista de veículos.","Vehicles")
	end
	if Keys["Propertys"][1] then
		for _,v in pairs(Keys["Propertys"]) do
			local Coords = exports["propertys"]:Coords(v.Number)["Coords"]
			local Road = GetStreetNameFromHashKey(GetStreetNameAtCoord(Coords["x"],Coords["y"],Coords["z"]))
			exports["dynamic"]:AddButton(Road.." Nº"..v.Number,"Criar chave cópia por <green>R$"..parseFormat(1000).."</green> Reais.","inventory:Key","1000-homekey-"..v.Keychain.."-"..v.Number,"Propertys",true)
		end
		exports["dynamic"]:SubMenu("Propriedades","lista de propriedades.","Propertys")
	end
	if Keys["Propertys"][1] or Keys["Vehicles"][1] then
...
-- INVENTORY:PROPERTYS
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("inventory:Propertys",function(Item)
	local Propertys = vPROPERTY.Propertys()
	if Propertys[1] then
		for Index in pairs(Propertys) do
			local Value = exports["propertys"]:Coords(Propertys[Index].Number)
			local Road = GetStreetNameFromHashKey(GetStreetNameAtCoord(Value.Coords["x"],Value.Coords["y"],Value.Coords["z"]))
			exports["dynamic"]:SubMenu(Road.." Nº"..Propertys[Index].Number,"Informações sobre a propriedade.",tostring(Propertys[Index].Number))
			exports["dynamic"]:AddButton("Transferência","Mudar proprietário.","propertys:Transfer",Propertys[Index].Number,tostring(Propertys[Index].Number),true)
			exports["dynamic"]:AddButton("Vender","Se desfazer da propriedade.","propertys:Sell",Propertys[Index].Number,tostring(Propertys[Index].Number),true)
			exports["dynamic"]:AddButton("Taxas","Pagar as taxas da propriedade.","propertys:Tax",Propertys[Index].Number,tostring(Propertys[Index].Number),false)
		end
		exports["dynamic"]:openMenu()
	end
end)
