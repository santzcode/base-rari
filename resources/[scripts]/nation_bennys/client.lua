...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			...
			if distance < 2 then
				drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~MECÂNICA",4,0.5,0.93,0.50,255,255,255,120)
...
RegisterNUICallback("pagar",function(data)
	if cart["total"] and func.checkPayment(cart["total"]) then
		...
		TriggerEvent("Progress",10000,"Aplicando Modificacoes")
		Wait(10000)
		...
		-- elseif config.use_vehlist then
			_,_,vehplate, vehname = vRP.VehicleList(7)
			while vehplate ~= GetVehicleNumberPlateText(vehicle) do
				_,_,vehplate, vehname = vRP.VehicleList(7)
				Wait(500)
			end
	...
	if data then
		if data.type == "reparar" then
			...
			if func.repairVehicle(vehicle,damage) then
				...
				myveh.damage = 0.0
				vRP.stopAnim(false)
				TriggerEvent("Notify","verde","Veículo reparado com <b>sucesso</b>.",7000)
...
AddEventHandler("nation:applymods", function(veh,vname,vehplate)
	if veh then
