	...
	window.addEventListener("message",function(event){
		...
		if (item["addbutton"] == true){
			if(item.id == false || null){
				...
				var b = (`<div id="normalbutton-${normalButtons}" data-trigger="`+item["trigger"]+`" data-parm="`+item["par"]+`" data-server="`+item["server"]+`" class="normalbutton ${item["title"] == "Guardar" ? "amarelo":""}"><div class="title">`+item["title"]+`</div>`+item["description"]+`</div>`);
				$(".Container").append(b);
				buttons.push(b);
			} else {
				var b = (`<button id="`+item["id"]+`" data-trigger="`+item["trigger"]+`" data-parm="`+item["par"]+`" data-server="`+item["server"]+`" class="a btn"><div class="title">`+item["title"]+`</div>`+item["description"]+`</button>`);
				buttons.push(b);
			}
		} else if(item["addmenu"] == true){
			var aa = (`<button data-menu="`+item["menuid"]+`" class="b btn"><div class="title">`+item["title"]+`</div>`+item["description"]+`<i class="fas fa-chevron-right" style="float:right;margin-top:-10%"></i></button>`)
			$(".Container").append(aa);
			submenus.push(aa);
		}
