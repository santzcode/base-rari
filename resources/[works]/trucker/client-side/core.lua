...
AddEventHandler("trucker:Vehicles",function()
	if not Package then
		...
		Package = true
		Service = "Vehicles"
		TriggerEvent("Notify","amarelo","Dirija-se ao caminhão e buzine o mesmo<br>para receber a carga responsável pelo transporte.",5000,"Aviso")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- TRUCKER:DIESEL
							...
							if Position == 1 then
								...
								if Position == 2 then
									if IsControlJustPressed(1,38) then
										...
										if hasTrailer and GetEntityArchetypeName(Trailer) == Delivery[Service]["Trailer"] then
											TriggerEvent("Notify","amarelo","Volte para receber o pagamento.",5000,"Atenção")
											TriggerServerEvent("garages:deleteVehicle",VehToNet(Trailer),GetVehicleNumberPlateText(Trailer))
											Position = Position + 1
											BlipMarked()
										end
									end
