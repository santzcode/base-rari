...
AddEventHandler(
    ...
    function(amount)
        PlayerOwnedChips = tonumber(amount) or 0
    end
)
...
Citizen.CreateThread(function()
    while true do
        if blackjackInstructional ~= nil then 
            ...
            DrawAdvancedNativeText(0.215, 0.840, 0.005, 0.0028, 0.464, string.format('%s', frm_GroupDigits(tostring(PlayerOwnedChips))), 255, 255, 255, 255, 0, 0)
			end
        end
...
function dealerGiveCards(chairId,gender,dealerPed,cardObj)
...
function frm_GroupDigits(value)
    -- Ensure value is converted to string and is valid
    local str_value = tostring(value)
    if str_value == nil or str_value == "" then
        return "0"
    end
    
    local left,num,right = string.match(str_value,'^([^%d]*%d)(%d*)(.-)$')
