...
local function CreateCasinoBlip()
    local casinoCoords = vector3(935.8365, 46.94, 80.09)
    
    casinoBlip = AddBlipForCoord(casinoCoords.x, casinoCoords.y, casinoCoords.z)
    SetBlipSprite(casinoBlip, 619) -- Casino icon
    SetBlipDisplay(casinoBlip, 4)
    SetBlipScale(casinoBlip, 1.2)
    SetBlipAsShortRange(casinoBlip, false)
    SetBlipColour(casinoBlip, 5) -- Gold color
    SetBlipPriority(casinoBlip, 10)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Casino Royal")
    EndTextCommandSetBlipName(casinoBlip)
    
    -- Add pulsing effect
    SetBlipFade(casinoBlip, 1, false)
end

Citizen.CreateThread(function()
    Wait(2000) -- Wait for map to load
    CreateCasinoBlip()
end)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        for i, location in ipairs(positions) do
            ...
            -- Casino entrance has special marker (bigger and more visible)
            local markerSize = i == 1 and 1.2 or 0.8
            local markerlAlpha = i == 1 and 100 or 50

            DrawMarker(27, loc1.x, loc1.y, loc1.z + 0.0, 0, 0, 0, 0, 0, 0, markerSize, markerSize, markerSize, red, green, blue, markerlAlpha, false, true, 2, false, nil, nil, false)
            DrawMarker(27, loc2.x, loc2.y, loc2.z + 0.5, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, red, green, blue, 50, false, true, 2, false, nil, nil, false)

            -- Check distances more efficiently
            local dist1 = GetDistanceBetweenCoords(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, true)
            ...
            if dist1 < 3.0 then
                ...
                if dist1 < 2.0 then
                    if i == 1 then
                        DrawText3Ds(loc1.x, loc1.y, loc1.z + 0.2, "~g~Pressione ~w~E~g~ para entrar no Casino Royal")
                        drawNativeNotification('Pressione ~INPUT_PICKUP~ Para ~g~entrar~s~ no ~y~Casino Royal~s~')
                    else
            ...
            elseif dist2 < 3.0 then
                ...
                if dist2 < 2.0 then
                    if i == 1 then
                        DrawText3Ds(loc2.x, loc2.y, loc2.z + 0.7, "~r~Pressione ~w~E~r~ para sair do Casino Royal")
                        drawNativeNotification('Pressione ~INPUT_PICKUP~ Para ~r~sair~s~ do ~y~Casino Royal~s~')
                    else
        ...
        inTeleportZone = isNearTeleport
        
        -- Dynamic wait time based on proximity
        if not isNearTeleport then
