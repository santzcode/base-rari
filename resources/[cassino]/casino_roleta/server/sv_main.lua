...
function getPlayerChips(source)
    ...
    if passport then
        ...
        local chips = vRP.InventoryItemAmount(source, "chips")
        ...
        if chips and chips > 0 then
            TriggerClientEvent('casino:nui:updateChips', source, chips)
            return chips
        end
    end

    TriggerClientEvent('casino:nui:updateChips', source, 0)
    return 0
end

RegisterNetEvent('casino:requestChips')
AddEventHandler('casino:requestChips', function()
    local source = source
    getPlayerChips(source)
end)
...
AddEventHandler('server_remote:rulett:taskSitDown',
...
AddEventHandler('casino:taskStartRoulette',
...
AddEventHandler('casino:rulett:notUsing',
...
function CheckWinners(bets, WinningBetIndex)
    ...
    for i = 1, #bets, 1 do
        ...
        if PLAYER_HANDLE then
            ...
            if (WinningBetIndex == '0' and betData.betId == '37') or (WinningBetIndex == '00' and betData.betId == '38') then
            ...
            elseif betData.betId <= '36' and WinningBetIndex == betData.betId then
                giveWinningChips(targetSrc, betData.betAmount, 35)
                playersWon[targetSrc] = true
                if playersLoss[targetSrc] then
                    playersWon[targetSrc] = nil
...
AddEventHandler('casino:taskBetRulett',
    function(rulettIndex, betId, betAmount)
        ...
        if aktivRulettek[rulettIndex] ~= nil then
            ...
            if betAmount then
                ...
                TriggerClientEvent('casino:nui:updateChips', source)
            ...
            TriggerClientEvent('Notify', source, 'vermelho', 'mesa de roleta inexistente no servidor.')
        end
    end
)
