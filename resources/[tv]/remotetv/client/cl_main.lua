...
local function EnableRemoteControl(enable)
    if CURRENT_SCREEN and #(GetEntityCoords(PlayerPedId()) - CURRENT_SCREEN.coords) <= Config.DistanceToTv then
        if enable then
            ...
            -- Verifica se a TV já está funcionando
            ...
            -- Sempre executa a animação ao fechar
        ...
        print('Você não está perto de uma TV.')
    end
end
...
RegisterKeyMapping(Config.Command, 'Abrir/Fechar Controle Remoto', 'keyboard', Config.Keybind)
