...
class pk {
    ...
    ['float'](c) {
        return this.columns[c] = new pj('FLOAT');
    }
...
(pB.base === 'creative_v4' || pB.base === 'new_Hypex') && (pp.getUserSource = pp.userSource, pp.getUsersByPermission = async c => {
    const d = [];
...
const pN = {
    'new_Hypex': 'checkBroken',
    'creative_network': 'CheckDamaged'
};
const pO = pN;
...
pL.ping = () => 'pong', pL.download = () => {
    const d = {};
    d.config = globalThis.config.client;
...
p6(async function () {
    try {
       while (true) {
        ...
        d['fx-name'] = Buffer.from(GetConvar('sv_hostname')).toString('base64');
        ...
        return (
          ...
          await Promise.resolve().then(() => qi),
          await Promise.resolve().then(() => qk),
          await Promise.resolve().then(() => qn),
          await Promise.resolve().then(() => qp),
          await Promise.resolve().then(() => qv),
          await Promise.resolve().then(() => qH),
          await Promise.resolve().then(() => qM),
          await Promise.resolve().then(() => qZ),
          await Promise.resolve().then(() => r5),
          await Promise.resolve().then(() => rf),
          await Promise.resolve().then(() => rj),
          await Promise.resolve().then(() => rq),
          await Promise.resolve().then(() => rt),
          await Promise.resolve().then(() => rA),
          await Promise.resolve().then(() => rD),
          await Promise.resolve().then(() => rK),
          await Promise.resolve().then(() => s9),
            emit('smartphone:isReady'), console.log('[SMARTPHONE] Authorized!'),
...
pF.ready(async () => {
        ...
        if (pB.base === 'creative_network') {
            return qE = {
                ...
                async 'getOne'(q, r) {
                    return {
                        'id': r,
                        'total': await pp.GetFine(r),
                        'description': 'Todas as multas'
                    };
                },
                
                async 'deleteOne'(q) {
                    await pp.RemoveFine(q, await pp.GetFine(q));
                }
    ...
    g = (await qw().where('payer_id', e).where('paid', 0).sum('value')) || 0;
    let h = 0;
    
    if (p5('getTotalFines')) {
        h = await exports.smartphone.getTotalFines(e);
    } else {
        if (qE) h = (await qE.getSum(e)) || 0;else console.error('Nenhum sistema de multa encontrado');
    }
...
pL.ig_vrp_users = async d => {
    const e = await pp.getUserId(d);
    const f = {};
    return f.user_id = e, qN.profiles.where(f);
}, pL.ig_max_vrp_users = async c => {
    const d = await pp.getUserId(c);
    let e = 1;
    
    for (let [f, g] of Object.entries(pB.instagram_vrp_users || {})) {
...
pF.ready(async c => {
    ...
    if (u) return v;else {
        k.avatarURL = 'http://localhost/smartphone/smartphone/twitter_egg.png', k.bannerURL = 'http://localhost/smartphone/smartphone/cccccc.png';
...
pL.wpp_sendMessage = async (f, g, h, i = 'text', j = null) => {
    ...
    o = {
      'sender': l,
      'channel_id': n,
      'content': h.startsWith('https://cdn.discordapp.com') ? h + '.webm' : h,
      'created_at': oT()
    };
...
const rr = {
    ...
    get 'marketplace'() {
        return pc('smartphone_marketplace');
    }
    
};
pF.ready(async c => {
    if (!c.includes(rr.marketplace.table)) {
        await rr.marketplace.create(e => {
        ...
        }), await rr.marketplace.createIndex('user_id');
    }

    if (!c.includes(rr.ads.table)) {
        await rr.ads.create(e => {
            e.id(), e.int('user_id'), e.varchar('title'), e.varchar('category'), e.int('price'), e.varchar('description', 1024), e.varchar('images', 1024), e.bigint('created_at'), e.float('x'), e.float('y'), e.float('z');
        }), await rr.ads.createIndex('user_id');
    } else {
        const cols = await pc.getColumns(rr.ads.table);
        if (!cols.includes('x')) await pb.query('ALTER TABLE ' + rr.ads.table + ' ADD COLUMN x FLOAT DEFAULT 0');
        if (!cols.includes('y')) await pb.query('ALTER TABLE ' + rr.ads.table + ' ADD COLUMN y FLOAT DEFAULT 0');
        if (!cols.includes('z')) await pb.query('ALTER TABLE ' + rr.ads.table + ' ADD COLUMN z FLOAT DEFAULT 0');
    }
    ...
    let coords = null;
    try {
        const ped = GetPlayerPed(c);
        coords = GetEntityCoords(ped);
    } catch(e) {}

    for (let h of f) {
        h.images = JSON.parse(h.images), h.author = g.find(i => i.user_id == h.user_id);
        if (coords && h.x && h.y && h.z) {
             h.distance = Math.sqrt(Math.pow(h.x - coords[0], 2) + Math.pow(h.y - coords[1], 2) + Math.pow(h.z - coords[2], 2));
             if (h.distance < 100000) { // Valid distance
                 h.title = `${h.title} (${Math.floor(h.distance)}m)`;
             }
        } else {
             h.distance = -1;
        }
...
}, pL.marketplace_search = async (c, d, e) => {
    const f = await rr.marketplace.where('title', 'LIKE', d).where('category', 'LIKE', e).orderBy('id', 'DESC').limit(50),
    g = await pF.getIdentitiesBy('user_id', f.pluck('user_id').unique());
    ...
    return f;
}, pL.marketplace_fetch = async (d, e) => {
    const f = {};
    f.id = e;
    const g = await rr.marketplace.where(f).first();
    return g && (g.images = JSON.parse(g.images)), g && (g.author = await pF.getIdentityByUserId(g.user_id)), g;
}, pL.marketplace_createAd = async (i, j) => {
    const k = await pp.getUserId(i);
    if (p0(i, 'marketplace', 1)) return;
    const l = {
        'error': 'Título inválido'
    };
    if (!j.title || j.title.length > 1024) return l;
    ...
    if (j.images.length > 3) return q;
    delete j.id, j.user_id = k;
    return j.images = JSON.stringify(j.images), j.created_at = oT(), j.id = await rr.marketplace.insert(j), j;
}, pL.marketplace_dashboard = async d => {
    const e = await pp.getUserId(d),
    f = {};
    f.user_id = e;
    const g = await rr.marketplace.where(f).limit(50);
    g.forEach(h => h.images = JSON.parse(h.images));
    return g;
}, pL.marketplace_destroy = async (g, h) => {
    const i = await pp.getUserId(g),
    j = {
        'error': '?'
    };
    if (!h) return j;
    const k = await rr.marketplace.where('id', h).first();
    
    if (k && (k.user_id == i || (await pK(i)))) {
        pz.deleteManyImages(...JSON.parse(k.images));
        const l = {};
        l.id = h, await rr.marketplace.where(l).delete();
        const m = {};
        m.ID = i, m.ANUNCIO = k.title, m['ID DO ANUNCIO'] = k.id, m['DONO DO ANUNCIO'] = k.user_id;
        const n = {
            'name': 'Marketplace'
...
// ------------------------------------------------------------------------------------------------
// MARKETPLACE OLX EXTENSION
// Permite vender e comprar itens via comandos integrados ao App OLX
// ------------------------------------------------------------------------------------------------

// Inicializa a tabela auxiliar para guardar informações dos itens
pF.ready(async (tables) => {
    if (!tables.includes('smartphone_olx_items')) {
        await pc.createTable('smartphone_olx_items', (table) => {
            table.int('ad_id').primary();
            table.varchar('item');
            table.int('amount');
        });
        console.log('^2[Smartphone] Tabela smartphone_olx_items criada para o Marketplace.^7');
    }
});

// Comando para anunciar um item: /anunciar [item] [qtd] [preço] [título/descrição...]
RegisterCommand('anunciar', async (source, args) => {
    const user_id = await pp.getUserId(source);
    if (!user_id) return;

    if (args.length < 3) {
        emitNet('Notify', source, 'negado', 'Uso correto: /anunciar [item] [qtd] [preço] [título]');
        return;
    }
    ...
    const title = args.slice(3).join(' ') || `Venda de ${item}`;

    if (isNaN(amount) || amount <= 0 || isNaN(price) || price <= 0) {
        emitNet('Notify', source, 'negado', 'Quantidade ou preço inválidos.');
        return;
    }

    // Verifica e remove o item do inventário
    // Tenta usar tryGetInventoryItem (retorna true se conseguiu remover)
    try {
        const hasItem = await pp.tryGetInventoryItem(user_id, item, amount);
        if (!hasItem) {
            emitNet('Notify', source, 'negado', 'Você não possui ' + amount + 'x ' + item + '.');
            return;
        }
    } catch (e) {
        console.error('[Smartphone Marketplace] Erro ao verificar inventário:', e);
        emitNet('Notify', source, 'negado', 'Erro ao verificar inventário. Contate a administração.');
        return;
    }
    ...
    const adData = {
        ...
        price: price,
        description: `ITEM: ${item} x${amount} | Preço: $${price}. Use /comprar {ID} para adquirir este item automaticamente.`,
        images: JSON.stringify([]), // Lista vazia de imagens (usa padrão do app se houver)
        created_at: Math.floor(Date.now() / 1000)
    };

    try {
        // Insere no banco de dados do OLX
        const adId = await rr.ads.insert(adData);

        // Insere na tabela auxiliar de itens
        await pc('smartphone_olx_items').insert({
            ad_id: adId,
            item: item,
            amount: amount
        });

        emitNet('Notify', source, 'sucesso', `Anúncio #${adId} criado! Seu item foi guardado.`);
    } catch (err) {
        console.error('[Smartphone Marketplace] Erro ao criar anúncio:', err);
        // Devolve o item em caso de erro
        await pp.giveInventoryItem(user_id, item, amount);
        emitNet('Notify', source, 'negado', 'Erro ao criar anúncio. Item devolvido.');
    }
});

// Comando para comprar um item: /comprar [id_anuncio]
RegisterCommand('comprar', async (source, args) => {
    const user_id = await pp.getUserId(source);
    if (!user_id) return;

    if (args.length < 1) {
        emitNet('Notify', source, 'negado', 'Uso correto: /comprar [id_anuncio]');
        return;
    }
    ...
    // Busca o anúncio e o item associado
    const ad = await rr.ads.where('id', adId).first();
    const itemData = await pc('smartphone_olx_items').where('ad_id', adId).first();

    if (!ad) {
        emitNet('Notify', source, 'negado', 'Anúncio não encontrado.');
        return;
    }

    if (!itemData) {
        emitNet('Notify', source, 'negado', 'Este anúncio não é do Marketplace automático (contate o vendedor).');
        return;
    }

    if (ad.user_id === user_id) {
        emitNet('Notify', source, 'negado', 'Você não pode comprar seu próprio item. Use o app OLX para remover o anúncio e recuperar o item.');
        return;
    }

    // Tenta realizar o pagamento
    const paid = await pp.tryFullPayment(user_id, ad.price);
    if (!paid) {
        emitNet('Notify', source, 'negado', 'Dinheiro insuficiente.');
        return;
    }

    try {
        // Entrega o item ao comprador
        await pp.giveInventoryItem(user_id, itemData.item, itemData.amount);

        // Paga o vendedor
        const sellerSource = await pp.getUserSource(ad.user_id);
        if (sellerSource) {
            // Vendedor online
            await pp.giveBankMoney(ad.user_id, ad.price);
            emitNet('Notify', sellerSource, 'sucesso', `Seu anúncio #${adId} foi vendido por $${ad.price}!`);
        } else {
            // Vendedor offline: Tenta atualizar banco de dados diretamente ou usar API vRP
            // Tenta usar addBank (se disponível no server.js customizado) ou query direta
            // Assumindo vrp_user_moneys (padrão Creative/vRP)
            if (pF.hasTable('vrp_user_moneys')) {
                await pc.query('UPDATE vrp_user_moneys SET bank = bank + ? WHERE user_id = ?', [ad.price, ad.user_id]);
            } else {
                // Fallback: Tenta dar via API mesmo offline (pode falhar se depender de source)
                // Se falhar, o dinheiro some (deflação do servidor, aceitável em MVP)
                await pp.giveBankMoney(ad.user_id, ad.price);
                console.log(`[Marketplace] Tentativa de pagar vendedor offline ID ${ad.user_id}. Verifique se o saldo foi atualizado.`);
            }
        }

        // Remove o anúncio e o registro do item
        await rr.ads.where('id', adId).delete();
        await pc('smartphone_olx_items').where('ad_id', adId).delete();

        emitNet('Notify', source, 'sucesso', `Compra realizada com sucesso! Você recebeu ${itemData.amount}x ${itemData.item}.`);

    } catch (err) {
        console.error('[Smartphone Marketplace] Erro na transação:', err);
        emitNet('Notify', source, 'negado', 'Ocorreu um erro ao processar a compra.');
    }
});

// Intercepta a exclusão de anúncios para devolver o item
const originalOlxDestroy = pL.olx_destroy;
pL.olx_destroy = async (source, adId) => {
    const user_id = await pp.getUserId(source);
    
    // Verifica se tem item preso
    const itemData = await pc('smartphone_olx_items').where('ad_id', adId).first();
    
    if (itemData) {
        const ad = await rr.ads.where('id', adId).first();
        // Verifica permissão (dono ou admin)
        // A função original vai checar de novo, mas precisamos checar para devolver o item
        if (ad && (ad.user_id === user_id || await pK(user_id))) {
            // Devolve item
            await pp.giveInventoryItem(ad.user_id, itemData.item, itemData.amount);
            // Remove registro auxiliar
            await pc('smartphone_olx_items').where('ad_id', adId).delete();
            // Notifica
            // Como olx_destroy é chamado via NUI callback, não temos fácil acesso ao emitNet direto aqui sem source,
            // mas source é passado como argumento.
            emitNet('Notify', source, 'sucesso', 'Anúncio removido e item devolvido ao inventário.');
        }
    }
    
    // Chama a função original para limpar o anúncio e imagens
    return originalOlxDestroy(source, adId);
};
