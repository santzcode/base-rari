		...
		if LocalPlayer["state"]["Active"] and Display then
			...
			if IsPedInAnyVehicle(Ped) then
				...
				if VEngineHealth ~= EngineHealth then
					SendNUIMessage({ engine = parseInt(VEngineHealth / 10) })
					VEngineHealth = EngineHealth
				end

				if Locked ~= VLocked then
					SendNUIMessage({ isLock = VLocked == 2 or false })
					Locked = VLocked
				end
				...
				if LocalPlayer["state"]["Nitro"] then
					SendNUIMessage({ nosValue = parseInt(NitroFuel / 20) })
					Nitro = NitroFuel
				else
				...
				if Fuel ~= VFuel then
					SendNUIMessage({ fuel = parseInt(VFuel) })
					Fuel = VFuel
				end

				if Speed ~= VSpeed then
					SendNUIMessage({ km = parseInt(VSpeed) })
					Speed = VSpeed
				end
				...
				if Rpm ~= VRpm then
					SendNUIMessage({ rpm = parseInt(VRpm * 100) * 3 })
					Rpm = VRpm
				end
				...
				if ActualVehicle then
					...
					Headbeams = false
					Headlights = false
					SendNUIMessage({ name = "Headlight", payload = 0 })

					Nitro = 0
					SendNUIMessage({ nosValue = 0 })
...
function CreatePurgeSprays(Vehicle,xOffset,yOffset,zOffset,xRot,yRot)
	UseParticleFxAssetNextCall("core")
	return StartParticleFxNonLoopedOnEntity("ent_sht_steam",Vehicle,xOffset,yOffset,zOffset,xRot,yRot,0.0,0.5,false,false,false)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SPACEENABLE
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if LocalPlayer["state"]["Active"] then
			...
			if IsPedInAnyVehicle(Ped) then
				if not IsPedOnAnyBike(Ped) and not IsPedInAnyHeli(Ped) and not IsPedInAnyPlane(Ped) then
					...
					if Speed ~= SeatbeltSpeed then
						if (SeatbeltSpeed - Speed) >= 60 and not SeatbeltLock then
							...
							TriggerEvent("creative:Airbag",Ped,vehicle)
				...
				if SeatbeltLock then
					SendNUIMessage({ isSeatBelt = false })
					SeatbeltLock = false
				end
	...
	if IsPedInAnyVehicle(Ped) then
		if not IsPedOnAnyBike(Ped) and not IsPedInAnyHeli(Ped) and not IsPedInAnyPlane(Ped) then
			if SeatbeltLock then
				...
				SendNUIMessage({ isSeatBelt = true })
				SeatbeltLock = true
			end
		end
	end
