...
Citizen.CreateThread(function ()
    for k,v in pairs(Config.Zones) do
        exports["target"]:AddCircleZone("Hookers:"..k,vec3(v.Pos.x, v.Pos.y, v.Pos.z+1.0),0.5,{
            name = "Hookers:"..k,
        },{
            Distance = 1.75,
            options = {
                {
                    event = "sawu_hookers:OpenPimpMenu",
                    label = "Abrir",
                    tunnel = "client"
                }
            }
...
Citizen.CreateThread(function()
    for k,v in pairs(Config.Zones) do
			...
			AddTextComponentString("Acompanhantes")
...
function signalHooker()
    ...
    SetBlockingOfNonTemporaryEvents(Hooker, true)
    while not IsPedInAnyVehicle(Hooker) do
        TaskEnterVehicle(Hooker, GetVehiclePedIsIn(ped, false), -1, 0, 1.0, 1, 0)
        Wait(1000)
    end
    TriggerEvent("Notify","azul","<b>Dirija para um lugar seguro</b>.",5000)	
end
-------------------------------------------------------------
