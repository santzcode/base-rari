Citizen.CreateThread(function()
  ...
  API.prepare('uber/create', [[CREATE TABLE IF NOT EXISTS smartphone_uber_trips(
  ...
  API.prepare('uber/insert', [[INSERT INTO smartphone_uber_trips
  ...
  API.prepare('uber/find_by_id', 'SELECT * FROM smartphone_uber_trips WHERE id=@id')
  API.prepare('uber/set_rate', 'UPDATE smartphone_uber_trips SET user_rate=@user_rate,driver_rate=@driver_rate WHERE id=@id')
  API.prepare('uber/get_user_rate', 'SELECT AVG(user_rate) as rate FROM smartphone_uber_trips WHERE user_id=@user_id AND user_rate > 0')
  API.prepare('uber/get_driver_rate', 'SELECT AVG(driver_rate) as rate FROM smartphone_uber_trips WHERE driver_id=@user_id AND driver_rate > 0')
  API.prepare('uber/get_trips', 'SELECT * FROM smartphone_uber_trips WHERE user_id=@user_id ORDER BY created_at DESC LIMIT 15')
  API.prepare('uber/get_driver_trips', 'SELECT * FROM smartphone_uber_trips WHERE driver_id=@user_id ORDER BY created_at DESC LIMIT 15')

  API.prepare('uber/get_trips_count', 'SELECT SUM(user_id=@user_id) AS passenger, SUM(driver_id=@user_id) AS driver FROM smartphone_uber_trips')
  API.execute('uber/create')
end)
...
function isTripTaken(id)
  return trips[id] or #API.query('uber/find_by_id', { id=id }) > 0
end
...
local function getRate(user_id, is_driver)
  ...
  return tonumber(API.query(name, { user_id = user_id })[1].rate)
end
...
local function getName(user_id)
  local id = API.getIdentity(user_id) or {}
  return id.name or id.nome or id.Name or '(Erro)'
end

local function getPhone(user_id)
  local id = API.getIdentity(user_id) or {}
  return id.phone or id.phone_number or id.telefone or id.Phone
end
...
expose('uber_create_trip', function(source, pos, distance)
  ...
  local balance = toInt(API.getBank(user_id))
  ...
  if API.subBank then
    API.subBank(user_id, price)
  else
    API.setBank(user_id, balance - price)
  end
  ...
  local trip = {
    ...
    customer = {
      ...
      avatarURL = 'https://cdn.jesteriruka.dev/stock/user_square.png',
    },
    push = function(self, ...)
    ...
    cancel = function (self, refund)
      ...
      if refund then
        API.addBank(self.customer.id, self.total)
      elseif self.driver then
        API.addBank(self.driver.id, self.total)
      end
    end,
    from = toXYZ(from),
    to = toXYZ(to),
  ...
  CreateThread(function()
    ...
    if not trip.driver then
      trips[trip.id] = nil
      API.addBank(user_id, trip.total)
    end
  end)
end)
...
expose('uber_my_trips', function(source, asDriver)
  local user_id = assert(API.getId(source), 'Você está bugado')
  local query = asDriver and 'uber/get_driver_trips' or 'uber/get_trips'
  local rows = API.query(query, { user_id = user_id })
...
expose('uber_set_rate', function(source, id, rate)
  ...
  local trip = assert(API.query('uber/find_by_id', { id = id })[1], 'Viagem não encontrada')
  ...
  API.execute('uber/set_rate', trip)
end)

expose('uber_profile', function(source, asDriver)
  local user_id = assert(API.getId(source), 'Você está bugado')
  local trips = API.query('uber/get_trips_count', { user_id = user_id })[1]
...
expose('uber_arrived', function(source)
  ...
  API.addBank(trip.driver.id, trip.total)
...
AddEventHandler('playerDropped', function()
  ...
  if trip then
    API.addBank(trip.customer.id, trip.total)
...
AddEventHandler('smartphone:isReady', function()
  exports.smartphone:createApp(
    'uber', 
    'Uber', 
    'nui://smartphone-jobs/build/icons/uber.webp',
